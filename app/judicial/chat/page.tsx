...
export default function JudicialChatPage() {
  ...
  async function send(overrideInput?: string) {
    ...
    const apiMessages = next.map((m, i) => {
      if (i === next.length - 1 && m.role === 'user' && selectedCaso) {
        return {
          role: 'user' as const,
          content: `[Contexto: expediente ${selectedCaso.numero_expediente}, alias ${selectedCaso.alias || 'N/A'}, id ${selectedCaso.id}]\n${m.content}`,
        };
      }
    ...
    try {
      ...
      const reply = data.text || data.message || data.error || 'Sin respuesta.';
      setMessages(prev => [...prev, {
        role: 'assistant',
        content: reply,
        isDocument: /SEÑOR JUEZ|PETITORIO|---DOCUMENTO_COMPLETO---/i.test(reply),
      }]);
    } catch {
  ...
  function downloadWordFromMessage(content: string) {
    const text = content.split('---DOCUMENTO_COMPLETO---')[0].trim();
    const blob = new Blob([text], { type: 'text/plain;charset=utf-8' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `escrito-chat-${Date.now()}.txt`;
    a.click();
        ...
        {messages.length === 0 && (
          <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', marginTop: '40px', marginBottom: '32px', flexWrap: 'wrap' }}>
            {CAPABILITY_PILLS.map(pill => (
              <button key={pill.label} type="button" onClick={() => void send(pill.prompt)}
            ...
            {m.role === 'assistant' ? (
              <div style={{ marginRight: '60px', background: '#f7f7f7', borderLeft: '3px solid #1a3d2b', padding: '16px 20px', maxWidth: '100%' }}>
                ...
                {(m.isDocument || isDocumentMsg(m.content)) && (
                  <button type="button" onClick={() => downloadWordFromMessage(m.content)}
      ...
      <div style={{ position: 'sticky', bottom: 0, background: '#ffffff', borderTop: '1px solid rgba(15,15,15,0.08)', padding: '16px 32px' }}>
        <div style={{ maxWidth: '760px', margin: '0 auto', position: 'relative' }}>
          {mentionDropdown && (
            <div style={{ position: 'absolute', bottom: '100%', left: 0, right: 0, background: 'white', border: '1px solid rgba(15,15,15,0.12)', boxShadow: '0 4px 12px rgba(0,0,0,0.08)', maxHeight: '200px', overflowY: 'auto', zIndex: 50, marginBottom: '4px' }}>
              {filteredCasos.length === 0 ? (
                ...
                <button key={c.id} type="button" onClick={() => selectCaso(c)}
          ...
          {selectedCaso && (
            <div style={{ marginBottom: '8px' }}>
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', background: 'rgba(26,61,43,0.1)', color: '#1a3d2b', padding: '4px 10px', fontFamily: 'DM Mono, monospace', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.08em', borderRadius: '2px' }}>
                ...
                <button type="button" onClick={removeSelectedCaso} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#1a3d2b', padding: '0', lineHeight: 1, fontSize: '14px' }}>×</button>
              </span>
            </div>
          )}
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            ...
            />
            <button type="button" onClick={() => void send()} disabled={isTyping || !input.trim()}
