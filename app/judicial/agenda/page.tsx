...
export default function JudicialAgendaPage() {
  ...
  if (loading) return <div style={{ padding: '48px 64px', fontFamily: 'var(--font-mono)', fontSize: '11px', textTransform: 'uppercase', color: 'var(--muted)' }}>Cargando agenda judicial...</div>;
  ...
  return (
    <div style={{ padding: '48px 64px', background: 'var(--paper)', minHeight: '100vh' }}>
      ...
      <h1 style={{ fontFamily: 'var(--font-body)', fontSize: 'clamp(32px, 4vw, 48px)', fontWeight: 600 }}>Agenda</h1>
      ...
      <div style={{ marginTop: '32px', display: 'flex', flexDirection: 'column', gap: '32px' }}>
        {sections.map(section => section.items.length > 0 && (
          <div key={section.label}>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.12em', color: 'var(--muted)', borderLeft: `3px solid ${section.color}`, paddingLeft: '12px', marginBottom: '14px' }}>
              {section.label}
            </div>
              ...
              {section.items.map((i, idx) => (
                ...
                >
                  <Link href={`/judicial/${i.casoId}`} style={{ flex: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'center', textDecoration: 'none', color: 'inherit', minWidth: 0 }}>
                    <div>
                      <div style={{ fontFamily: 'var(--font-body)', fontSize: '14px', fontWeight: 600 }}>{i.alias}</div>
                      <div style={{ fontFamily: 'var(--font-body)', fontSize: '13px', color: 'var(--muted)' }}>{i.descripcion}</div>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontFamily: 'var(--font-body)', fontSize: '13px' }}>{i.fecha}</div>
                      <div style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', color: 'var(--muted)' }}>{i.tipo || 'evento'} · {daysUntil(i.fecha)}d</div>
                    </div>
                  </Link>
                  <CalendarButtons title={i.alias} date={i.fecha} description={i.descripcion || 'Proceso judicial - arthur.ia'} />
                </div>
              ))}
            </div>
          </div>
