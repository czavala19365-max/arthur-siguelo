...
const ACCENT = 'rgba(194,164,109,'

function detectIntent(q: string): string {
  const t = q.toLowerCase()

  if (/agregar\s+t[ií]tulo|a[ñn]adir\s+t[ií]tulo|nuevo\s+t[ií]tulo|registrar\s+t[ií]tulo|seguimiento/.test(t)) {
    return '/dashboard/siguelo?action=agregar'
  }
  if (/agenda|plazo|vencimiento|fecha/.test(t)) {
    return '/dashboard/agenda'
  }
  if (/buscar\s+t[ií]tulo|consultar\s+t[ií]tulo|estado\s+(?:de\s+)?t[ií]tulo/.test(t)) {
    return '/dashboard/siguelo'
  }
  return `/dashboard/chat?q=${encodeURIComponent(q)}`
}

const quickChips = [
  { icon: '📋', label: 'Seguimiento de Títulos', href: '/dashboard/siguelo' },
  { icon: '📅', label: 'Agenda de Plazos',        href: '/dashboard/agenda' },
  { icon: '⚖️', label: 'Consulta Legal',           href: '/dashboard/chat'   },
]
  ...
  return (
    <>
      <style>{`
        ...
        .qbox:focus-within {
          border-color: ${ACCENT}0.55) !important;
          box-shadow: 0 4px 32px ${ACCENT}0.08), 0 2px 20px rgba(0,0,0,0.05) !important;
        }
        .quick-chip {
      ...
      >
        ...
        >
          {quickChips.map(c => (
            <Link key={c.href} href={c.href} className="quick-chip">
              <span>{c.icon}</span>
              <span>{c.label}</span>
            </Link>
          ))}
